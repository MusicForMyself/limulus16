...
// CREATE DEFAULT USERS ///////////////////////////////////////////////////////////////
	...
	function campos_personalizados_autores ( $user ) {
		...
		<table class="form-table">
			<tr>
				<th><label for="twitter">Colaborador</label></th>

				<td>
					<input type="checkbox" name="es_colaborador" id="es_colaborador" <?php echo $check; ?>/><br />

				</td>
			</tr>
			<tr>
				<th><label for="twitter">Socio</label></th>

				<td>
					<input type="checkbox" name="es_socio" id="es_socio" <?php echo $check_socio; ?>/><br />

				</td>
			</tr>
			<tr>
				<th><label for="twitter">Foto colaborador</label></th>
				<td>
					<input type="text" name="foto_colaborador" id="foto_colaborador" value="<?php echo esc_attr( get_the_author_meta( 'foto_colaborador', $user->ID ) ); ?>" class="regular-text" /><br />
			...
			<tr>
				...
				<td>
					<textarea rows="3" id="bio_en" name="_bio_es"><?php echo esc_attr( get_the_author_meta( '_bio_es', $user->ID ) ); ?></textarea>
	...
	function guarda_datos_extra_perfil_autor( $user_id ) {
		...
		if ( isset($_POST['es_colaborador']) ){
			update_user_meta( $user_id, 'es_colaborador', $_POST['es_colaborador'] );
		} else {
			delete_user_meta( $user_id, 'es_colaborador' );
		}

		if ( isset($_POST['es_socio']) ){
			update_user_meta( $user_id, 'es_socio', $_POST['es_socio'] );
		} else {
			delete_user_meta( $user_id, 'es_socio' );
		}
